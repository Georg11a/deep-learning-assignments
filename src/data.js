...
export const samples = ['1001','1002'].flatMap(speaker=>Object.entries(sentences).flatMap(([sentence,text])=>Object.entries(codes).map(([code,intended])=>({id:`${speaker}_${sentence}_${code}_XX`,speaker,text,intended,language:'English',source:'CREMA-D',group:`${speaker}_${sentence}`,url:`/audio/${speaker}_${sentence}_${code}_XX.wav`})))).filter(c=>c.id!=='1002_TIE_NEU_XX').map((c,i)=>({...c,sourceId:c.id,id:`sample-${String(i+1).padStart(2,'0')}`}));
